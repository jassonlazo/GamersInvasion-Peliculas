...
var play_rkmedia = false,
	...
	rk_light = false;
function onjwplayer(link,caption,nextid) {
    var plugins = "timeslidertooltipplugin,http://tvkhoahoc.com/template/player/plugins/proxy.swf,http://phim-vn.com/content/template/jwplayer/captions.swf",
        player = "http://nuiphim.com/play/player.swf",
        skin = "http://phim-vn.com/content/template/jwplayer/skin/skins.xml";
    jwplayer("rkplayer").setup({
        flashplayer: player,
        plugins: plugins,
        logo: "http://phim-vn.com/content/template/images/logoo.png", 
        "logo.opacity": "0.8",
        "logo.position": "top-right",
        "logo.hide": "false",
        "logo.margin" : "10",
        "abouttext": "Phim-VN.Com",
        "proxy.link": link,
        "proxy.image": 'http://phim-vn.com/content/template/images/cover.jpg',
        "stretching": "uniform",
        "controlbar": "bottom",
...
function load_datadownload(epid) {
	if(download_video == false) {
		$('.download-box').show().html("<img src='"+base_url+"/content/template/images/loading.gif'> Đang tải, vui lòng chờ trong giây lát ...");
		$.post(ajax_url, {
			RK_Download: 1,
			epid: epid
		}, function (data) {
			if(data) {
				$('.download-box').html('Chọn chất lượng phim sau đó nhấn chuột phải để lưu phim về máy<br />'+data);
				download_video = true;
			}
			else {
				play_rkmedia = true;
				var link = getVideoLink();
				if(link) $('.download-box').html("Nhấn chuột phải <a href='"+link+"' title='Tải video này' target='_blank'>vào đây</a> để lưu phim về máy.");
			}
		});
	}
	return false;
